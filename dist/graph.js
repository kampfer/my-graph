...
class NetworkGraph extends eventemitter3 {
    ...
    constructor({
        ...
        if (this.useDrag) {
            this.d3Drag = drag()
                .on('start', (event, d) => {
                    if (!event.active) simulation$1.alphaTarget(0.3).restart();   // 重新激活force tick
                    d.fx = d.x;
                    d.fy = d.y;
                })
                .on('drag', (event, d) => {
                ...
                .on('end', (event, d) => {
                    if (!event.active) simulation$1.alphaTarget(0);   // 动画可以停止
                    d.fx = null;
                    d.fy = null;
                });
        }
